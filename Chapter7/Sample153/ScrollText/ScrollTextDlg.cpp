...
void CScrollTextDlg::OnPaint() 
{
	...
	{
		CPaintDC dc(this); 
		dc.SetTextColor(RGB(18,0,210));//设置文本颜色
		dc.SetBkMode(TRANSPARENT);//设置透明背景
		dc.TextOut(WidthX,45,str);//字幕初始位置
...
void CScrollTextDlg::OnTimer(UINT nIDEvent) 
{
	...
	InvalidateRect(&rect);
	CRect rect2;
	GetClientRect(&rect2);//获取对话框客户窗口大小
	if(WidthX<-str.GetLength()*tm.tmAveCharWidth)//滚动到最左端
		WidthX=rect2.right;//从最右边开始显示
	WidthX=WidthX-6;
	UpdateWindow();//更新窗口
