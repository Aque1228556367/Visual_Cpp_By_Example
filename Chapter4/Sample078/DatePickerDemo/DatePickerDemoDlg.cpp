...
BOOL CDatePickerDemoDlg::OnInitDialog()
{
	...
	m_ctlDateTime.SetFormat("ddd',' MMM dd',' yyyy");//设定控件的显示格式
	//计算时间的上、下限
	...
	m_ctlDateTime.SetRange(&MinTime, &MaxTime);//设定控件可选的时间范围	
...
void CDatePickerDemoDlg::OnSet() 
{
	...
	DWORD result = m_ctlDateTime.GetRange(&MinTime, &MaxTime);//获取时间段
	msg = _T("\n可选时间的下限: ");
	if (result & GDTR_MIN) 
