...
BOOL CBitmapTreeDemoDlg::OnInitDialog()
{
	...
	m_ctlTree.SetBKImage("Blue hills.bmp"); //设置背景位图

	TVINSERTSTRUCT tvInsert;
	tvInsert.hParent = NULL;
	tvInsert.hInsertAfter = NULL;
	tvInsert.item.mask = TVIF_TEXT;
	tvInsert.item.pszText = _T("国防科技大学");
	
	HTREEITEM hCountry = m_ctlTree.InsertItem(&tvInsert);
	HTREEITEM hPA = m_ctlTree.InsertItem(TVIF_TEXT,
		_T("计算机学院"), 0, 0, 0, 0, 0, hCountry, NULL);
	
	HTREEITEM hWA = m_ctlTree.InsertItem(_T("电子科学与工程学院"),
		0, 0, hCountry, hPA);
		
	m_ctlTree.InsertItem(_T("计算机硬件基础"), hPA, TVI_SORT);
	m_ctlTree.InsertItem(_T("网络控制与安全"), hPA, TVI_SORT);
	m_ctlTree.InsertItem(_T("编译原理"), hPA, TVI_SORT);
	
	m_ctlTree.InsertItem(_T("遥感信息处理"), hWA, TVI_SORT);
	m_ctlTree.InsertItem(_T("雷达技术"), hWA, TVI_SORT);
	m_ctlTree.InsertItem(_T("通信技术"), hWA, TVI_SORT);
